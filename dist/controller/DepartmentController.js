...
export class DepartmentController {
        ...
        try {
            ...
            if (user.role !== "ADMINISTRADOR") {
                return res.status(403).json({
                    success: false,
                    message: "Apenas administradores podem cadastrar departamentos.",
                });
            }
            const department = await this.departmentService.create(req.body, user.id);
            ...
            if (error instanceof Error) {
                ...
                if (error.message ===
                    ...
                    return res.status(404).json({
                        success: false,
                        message: "Líder responsável não encontrado.",
                    });
                }
            }
    ...
    update = async (req, res) => {
        try {
            const department = await this.departmentService.update(req.params.id, req.body);
            return res.status(200).json(department);
        }
        catch (error) {
            console.error(error);
