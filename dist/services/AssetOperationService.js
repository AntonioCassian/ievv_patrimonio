...
export class AssetOperationService {
    ...
    async create(data, createdById) {
        return await prisma.$transaction(async (tx) => {
            ...
            if (creator.status !== "ACTIVE") {
                throw new Error("CREATOR_INACTIVE");
            }
            // =========================================
            // USUFRUTO
            // =========================================
            if (data.tipoOperacao === "USUFRUTO") {
                ...
                // CRIAR USUFRUTO
                // =========================================
                const operation = await tx.assetOperation.create({
                    data: {
                        type: "USUFRUTO",
                        status: "ACTIVE",
                        assetId: data.patrimonioId,
                        quantity: data.quantidade,
                        operationDate: new Date(data.dataOperacao),
                        observations: data.observacoes ??
                            null,
                        responsibleId: data.responsavelId,
                        startDate,
                        endDate,
                ...
                // =========================================
                const newQuantity = asset.quantityAvailable -
                    data.quantidade;
            ...
            // DOAÇÃO
            // =========================================
            if (data.tipoOperacao === "DOACAO") {
                if (!data.doador) {
                    throw new Error("DONOR_REQUIRED");
                }
                if (!data.dataDoacao) {
                    throw new Error("DONATION_DATE_REQUIRED");
                }
                // =========================================
                // CRIAR DOAÇÃO
                // =========================================
                const operation = await tx.assetOperation.create({
                    data: {
                        ...
                        observations: data.observacoes ??
                            null,
                        donor: data.doador,
                        donationDate: new Date(data.dataDoacao),
    ...
    // =========================================
    async findById(id) {
        const operation = await prisma.assetOperation.findUnique({
            where: {
                id,
            },
            include: {
    ...
    async update(id, data) {
        ...
        if (operation.type === "DOACAO") {
            throw new Error("DONATION_CANNOT_BE_UPDATED");
        }
    ...
    async delete(id) {
        return await prisma.$transaction(async (tx) => {
            const operation = await tx.assetOperation.findUnique({
                where: {
                    id,
                },
            });
            ...
            if (operation.type ===
                ...
                operation.status ===
                    "ACTIVE") {
                throw new Error("ACTIVE_OPERATION_CANNOT_BE_DELETED");
            }
    ...
    // RENOVAR USUFRUTO
    // =========================================
    async renewUsufruct(id, data, createdById) {
        return await prisma.$transaction(async (tx) => {
            // =========================================
            // BUSCAR OPERAÇÃO
            // =========================================
            ...
            if (operation.type ===
                "DOACAO") {
                throw new Error("DONATION_CANNOT_BE_RENEWED");
            }
            // =========================================
            // VERIFICAR STATUS
            // =========================================
            if (operation.status !==
                "ACTIVE") {
                throw new Error("USUFRUCT_NOT_ACTIVE");
            }
            // =========================================
            // VERIFICAR DATA ATUAL
            // =========================================
            if (!operation.endDate) {
                throw new Error("END_DATE_NOT_FOUND");
            }
            const newEndDate = new Date(data.novaDataTermino);
            ...
            // =========================================
            const previousEndDate = operation.endDate;
            // =========================================
            // ATUALIZAR USUFRUTO
            // =========================================
...
//# sourceMappingURL=AssetOperationService.js.map