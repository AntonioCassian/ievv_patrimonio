...
} from "../services/AssetOperationService.js";

export class AssetOperationController {
    private readonly service: AssetOperationService;
                ...
                switch (error.message) {
                    ...
                    case "DONATION_CANNOT_BE_UPDATED":
                        return res.status(409).json({
                            success: false,
                            message:
                                "Operações de doação não podem ser alteradas.",
                        });
            ...
            if (
                ...
                if (
                    ...
                    return res.status(409).json({
                        success: false,
                        message:
                            "Operações ativas não podem ser excluídas.",
                    });
                }
            }
    ...
    renew = async (
        ...
        try {
            ...
            const operation =
                await this.service.renewUsufruct(
                    req.params.id,
                    req.body,
                    user.id
                );
                ...
                switch (error.message) {
                    ...
                    case "DONATION_CANNOT_BE_RENEWED":
                        return res.status(409).json({
                            success: false,
                            message:
                                "Doações não podem ser renovadas.",
                        });
                    ...
                    case "END_DATE_NOT_FOUND":
                        return res.status(400).json({
                            success: false,
                            message:
                                "O usufruto não possui data de término.",
                        });
